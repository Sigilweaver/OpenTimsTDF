...
//! Provides:
//!
//! * [`TdfSource`] - a [`mass_spec_core::SpectrumSource`] adapter over an
//!   open [`Reader`]. Use this when you want to feed timsTOF data into any
//!   `mass-spec-core`-shaped consumer (column store ingest, Arrow bridge,
//!   ...).
//! * [`write_mzml`] / [`write_indexed_mzml`] - convenience entry points
//!   that wrap the canonical writer in `mass-spec-core`.
//!
...
/// [`iter_spectra`](mass_spec_core::SpectrumSource::iter_spectra) method.
///
...
/// build their own [`mass_spec_core::SpectrumSource`].
...
/// valid mzML 1.1.0 document via the canonical writer in `mass-spec-core`.
